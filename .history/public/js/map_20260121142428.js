...
(function() {
  ...
  // Undo History
  // ============================================================
  const undoHistory = []; // Stack of {type, id, layer} objects
  const MAX_UNDO_HISTORY = 50;

  function addToUndoHistory(featureType, featureId, layer) {
    undoHistory.push({ type: featureType, id: featureId, layer: layer });
  ...
  function updateUndoButton() {
    ...
    if (undoBtn) {
      ...
      undoBtn.title = undoHistory.length > 0 
        ? `Undo last action (${undoHistory.length} in history)` 
        : 'Nothing to undo';
    }
  }
  ...
  async function undoLastAction() {
    ...
    // Remove from map
    if (last.layer && drawnItems.hasLayer(last.layer)) {
    ...
    if (v) {
      if (last.type === 'area') {
        v.areas = v.areas.filter(a => a.id !== last.id);
      } else if (last.type === 'road') {
        v.roads = v.roads.filter(r => r.id !== last.id);
      } else if (last.type === 'marker') {
        v.markers = v.markers.filter(m => m.id !== last.id);
      }
    }
    ...
    try {
      let endpoint;
      if (last.type === 'area') {
        endpoint = `${SPATIAL_AREAS_URL}/${currentViewKey}/areas/${last.id}`;
      } else if (last.type === 'road') {
        endpoint = `${SPATIAL_ROADS_URL}/${currentViewKey}/roads/${last.id}`;
      } else if (last.type === 'marker') {
        endpoint = `${SPATIAL_MARKERS_URL}/${currentViewKey}/markers/${last.id}`;
      }
      ...
      if (endpoint) {
        ...
        if (res.ok) {
          updateStatus(`Undone: ${last.type} removed`);
        } else {
          updateStatus('Undo: removed from map (DB delete may have failed)', 'error');
        }
      }
    } catch (err) {
      console.error('Undo delete error:', err);
      updateStatus('Undo: removed from map (DB error)', 'error');
    }
  ...
  map.on('mousemove', function(e) {
    ...
    // Add every point for smooth curves
  ...
  map.on('mouseup', async function(e) {
    ...
    // Simplify only if we have too many points (light simplification to preserve curves)
    let finalPoints = freehandPoints;
    if (freehandPoints.length > 500) {
      // Very light simplification - just reduce density, preserve shape
      finalPoints = reducePointDensity(freehandPoints, 3); // Keep every 3rd point
    } else if (freehandPoints.length > 200) {
      finalPoints = reducePointDensity(freehandPoints, 2); // Keep every 2nd point
    }
    ...
    const finalPolyline = L.polyline(finalPoints, {
      color: '#ff7800',
      weight: 3
    });
    ...
    const geometry = {
      type: 'LineString',
      coordinates: finalPoints.map(p => [p[1], p[0]]) // [lng, lat]
    };
    ...
    if (roadId) {
      ...
      // Add to undo history
      addToUndoHistory('road', roadId, finalPolyline);
  ...
  // Simple point density reduction - keeps every nth point
  function reducePointDensity(points, n) {
  ...
  const GEOSERVER_URL = 'http://geoserversafe.duckdns.org:65437/geoserver'; //change this if needed - geoserver IP/DNS
  const GEOSERVER_WORKSPACE = 'gis_project';                                //change this if needed - geoserver project name
  ...
  map.on(L.Draw.Event.CREATED, async (e) => {
    ...
    if (layerType === 'polyline') {
      ...
      if (roadId) {
        ...
        // Add to undo history
        addToUndoHistory('road', roadId, layer);
    ...
    if (layerType === 'circlemarker') {
      ...
      if (markerId) {
        ...
        // Add to undo history
        addToUndoHistory('marker', markerId, layer);
    ...
    if (layerType === 'polygon' || layerType === 'rectangle') {
      ...
      if (areaId) {
        ...
        // Add to undo history
        addToUndoHistory('area', areaId, layer);
  ...
  if (undoBtn) {
    ...
    updateUndoButton(); // Set initial state
  }
