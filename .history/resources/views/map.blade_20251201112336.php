...
<body>
  ...
  function renderAllAreas(ms) {
    ...
    if (customRoads.length) {
      ...
      customRoads.forEach(road => {
        const row = document.createElement('div');
        row.className = 'road-pill';  // reuse same styling as normal road pills
        row.textContent = road.name || '(unnamed road)';

        // Optional: click to zoom to that custom road
        row.onclick = () => {
          try {
            ...
            console.error('Failed to zoom to custom road', e);
          }
        };
