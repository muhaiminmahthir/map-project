...
<body>
  ...
  map.on(L.Draw.Event.CREATED, async (e) => {
    ...
    const geometry = layer.toGeoJSON().geometry;
        ...
        // Ask user if they want to name this area
        let customName = prompt(
          'Give this area a name (optional, e.g. "Lot 12", "Corner shop"):'
        );
        if (customName) {
          customName = customName.trim();
          if (!customName.length) customName = null;
        }

        // Register a new area (note: customName stored here)
      ...
      renderAllAreas(ms);
      // Optional: auto-save after each new area
