...
<body>
  ...
  map.on(L.Draw.Event.CREATED, async (e) => {
    ...
    try {
      ...
      // Prefer the newer 'roads' array; fall back to old 'names' if needed
      const listData = data.roads || (data.names || []);
      const count    = data.count ?? (Array.isArray(listData) ? listData.length : 0);

      renderList(listData, count, ms);
  ...
  // ---------- list rendering ----------
  function renderList(items, count, ms) {
    const stats = document.getElementById('stats');
    stats.innerHTML = `<small>${count} roads found (${ms} ms)</small>`;

    const list = document.getElementById('list');
    list.innerHTML = '';

    // Normalize: accept either array of strings or array of {key,label}
    const roads = (items || []).map(item => {
    ...
    roads.forEach(road => {
      ...
      // stable key used for backend lookup (e.g. "__unnamed__" or "Jalan XYZ")
      pill.dataset.key = road.key;
      pill.dataset.label = road.label;

      // Click → highlight road by key
      pill.onclick = () => highlightRoad(road.key);

      // Double-click → allow local rename (does NOT change backend)
      pill.ondblclick = () => {
        const current = pill.dataset.label || pill.textContent;
        const renamed = prompt('Local name for this road:', current);
        if (renamed && renamed.trim()) {
          pill.dataset.label = renamed.trim();
          pill.textContent   = renamed.trim();
        }
      };
